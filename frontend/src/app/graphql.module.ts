...
export function createApollo(httpLink: HttpLink, authService: AuthService) {
  ...
  // Verificar se temos configurações no window.ENV (configurado em runtime)
  ...
  const errorLink = onError(({ graphQLErrors, networkError }) => {
    if (graphQLErrors) {
      graphQLErrors.forEach(({ message, locations, path }) => {
        console.error(
          ...
          // Remove detalhes sensíveis do log
          path ? `Path: ${path}` : ''
        );
      });
    }
    if (networkError) {
      ...
      // Remover detalhes completos do erro
    }
  });
    ...
    typePolicies: {
      Board: {
        fields: {
          ...
          // Adicionar política para createdAt
          createdAt: {
            read(createdAt) {
      ...
      Card: {
        fields: {
          // Adicionar política para dueDate
          dueDate: {
            read(dueDate) {
...
export class GraphQLModule {
  constructor() {
    // Desativar logs desnecessários durante a inicialização
